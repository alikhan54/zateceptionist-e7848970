...
interface SearchResult {
  ...
  category: string;
  minHierarchy?: number; // NEW: minimum role hierarchy required
  adminOnly?: boolean; // NEW: only master_admin can see
}

// Navigation items with permission requirements
const navigationItems: SearchResult[] = [
  // Core - Everyone can access
  {
    id: "dashboard",
    title: "Dashboard",
    url: "/dashboard",
  ...
  // Sales - Most accessible, some restricted
  {
    id: "sales-dashboard",
  ...
  {
    id: "auto-leadgen",
    title: "Auto Lead Generation",
    url: "/sales/auto-leadgen",
  ...
  // Settings - Various permission levels
  {
    id: "settings",
...
const quickActions = [
  ...
  },
  {
    id: "new-deal",
    title: "Create Deal",
    description: "Add a new sales deal",
    icon: <DollarSign className="h-4 w-4" />,
  },
...
export function CommandPalette() {
  ...
  // Get user's role hierarchy from usePermissions hook
  ...
  useEffect(() => {
    const checkMasterAdmin = async () => {
      try {
        ...
        const appMetadata = session.session.user.app_metadata;
        if (userMetadata?.role === "master_admin" || appMetadata?.role === "master_admin") {
          setIsMasterAdmin(true);
        }
      } catch (error) {
  ...
  const filteredNavigationItems = useMemo(() => {
    return navigationItems.filter((item) => {
      // Admin-only items - only master_admin can see
  ...
  // Group FILTERED navigation items by category
  const groupedItems = filteredNavigationItems.reduce(
    (acc, item) => {
    ...
    <>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        ...
        <CommandList>
          ...
          {/* Navigation Groups - Now filtered by permissions */}
          {Object.entries(filteredGroups).map(([category, items]) => (
            <CommandGroup key={category} heading={category}>
              {items.map((item) => (
                <CommandItem key={item.id} onSelect={() => handleSelect(item.url)} className="flex items-center gap-2">
                  <div className="flex h-6 w-6 items-center justify-center text-muted-foreground">{item.icon}</div>
