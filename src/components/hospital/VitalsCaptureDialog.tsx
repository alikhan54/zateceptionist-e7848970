...
// The vitals a nurse captures at the bedside, in order. key === clinic_visits column. lk = i18n label key.
const FIELDS: { key: keyof VitalsPayload; label: string; unit: string; step?: string; lk: string }[] = [
  { key: "blood_pressure_systolic", label: "BP Systolic", unit: "mmHg", lk: "vc.bp_sys" },
  { key: "blood_pressure_diastolic", label: "BP Diastolic", unit: "mmHg", lk: "vc.bp_dia" },
  { key: "heart_rate", label: "Pulse", unit: "bpm", lk: "vc.pulse" },
  { key: "spo2", label: "SpO₂", unit: "%", lk: "vc.spo2" },
  { key: "respiratory_rate", label: "Resp. rate", unit: "/min", lk: "vc.resp" },
...
export function VitalsCaptureDialog({
  ...
  async function save() {
    if (!patientId) { toast({ title: t("vc.noPatient"), variant: "destructive" }); return; }
    ...
    if (!any) { toast({ title: t("vc.enterOne"), variant: "destructive" }); return; }
    try {
      let vid = targetVisit;
      ...
      toast({ title: t("vc.saveFail"), description: e?.message || t("common.tryAgain"), variant: "destructive" });
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onOpenChange(false); }}>
      <DialogContent className="hx-dialog max-w-xl border p-0" style={{ background: "var(--hx-dialog-bg)", borderColor: "var(--hx-dialog-border)", color: "var(--hx-text)" }}>
        <DialogTitle className="sr-only">{t("vc.title")}</DialogTitle>
        <div className="hx" style={{ margin: 0, minHeight: 0, padding: "1.4rem 1.5rem 1.6rem", background: "transparent" }} data-testid="hx-vitals-dialog">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <div className="hx-eyebrow">{t("vc.eyebrow")}</div>
              <div className="hx-h1" style={{ fontSize: "1.2rem" }}>{t("vc.title")}{patientName ? ` — ${patientName}` : ""}</div>
            </div>
            <span className="ml-auto">
              {worst === "critical" && <span className="hx-chip hx-chip--crit"><AlertTriangle className="h-3 w-3" /> {t("vc.critical")}</span>}
              {worst === "warning" && <span className="hx-chip hx-chip--warn"><AlertTriangle className="h-3 w-3" /> {t("vc.watch")}</span>}
              {worst === "normal" && <span className="hx-chip hx-chip--ok"><CheckCircle2 className="h-3 w-3" /> {t("vc.inRange")}</span>}
            </span>
          </div>
          <p className="hx-dim text-sm mb-4">{t("vc.note")}</p>
            ...
            {FIELDS.map((fld) => {
              ...
              return (
                <div key={fld.key as string}>
                  <label className="hx-label">{t(fld.lk, fld.label)} <span className="hx-faint">{fld.unit}</span></label>
                  <div className="flex items-center gap-2">
                    ...
                    {st !== "empty" && st !== "normal" && (
                      <span className={`hx-chip ${st === "critical" ? "hx-chip--crit" : "hx-chip--warn"}`} style={{ padding: "0.05rem 0.4rem" }}>{st === "critical" ? t("vc.crit") : t("vc.highlow")}</span>
                    )}
                  </div>
                </div>
              );
          ...
          <div className="flex items-center justify-between gap-2 mt-5">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" className="hx-btn hx-btn--primary" onClick={save} disabled={busy} data-testid="hx-vc-save">
                {busy ? <><Loader2 className="h-4 w-4 animate-spin" /> {t("common.saving")}</> : <><HeartPulse className="h-4 w-4" /> {t("vc.record")}</>}
              </button>
            </div>
          </div>
        </div>
