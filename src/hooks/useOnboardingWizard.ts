...
export function useBusinessAnalysis() {
  ...
  const analyzeSocial = useCallback(async (
...
export function useBusinessProfile() {
  ...
  // Save profile
  const saveProfile = useCallback(async (profileData: Partial<BusinessProfile>) => {
    if (!tenantId) return;
...
export function useServicesCatalog() {
  ...
  // Add service
  const addService = useCallback(async (service: Partial<ServiceCatalogItem>) => {
    if (!tenantId) return;
  ...
  // Update service
  const updateService = useCallback(async (id: string, updates: Partial<ServiceCatalogItem>) => {
    try {
      const { data, error } = await supabase
        .from('services_catalog')
        .update(updates)
  ...
  // Bulk add services
  const bulkAddServices = useCallback(async (servicesList: Partial<ServiceCatalogItem>[]) => {
    if (!tenantId) return;
    
    try {
      const { data, error } = await supabase
        .from('services_catalog')
        .insert(servicesList.map((s, i) => ({
          tenant_id: tenantId,
          display_order: i,
          ...s,
        })))
...
export function useTargetPersonas() {
  ...
  // Add persona
  const addPersona = useCallback(async (persona: Partial<TargetPersona>) => {
    if (!tenantId) return;
  ...
  // Update persona
  const updatePersona = useCallback(async (id: string, updates: Partial<TargetPersona>) => {
    try {
      const { data, error } = await supabase
        .from('target_personas')
        .update(updates)
...
export function useIndustryTemplates() {
  ...
  const getTemplate = useCallback((industryCode: string) => {
    return templates.find(t => t.industry_code === industryCode) || 
           templates.find(t => t.industry_code === 'general');
  }, [templates]);
...
export function useOnboardingChecklist() {
  ...
  // Complete item
  const completeItem = useCallback(async (itemCode: string, completionData?: Record<string, any>) => {
    if (!tenantId) return;
  ...
  // Skip item
  const skipItem = useCallback(async (itemCode: string) => {
    if (!tenantId) return;
