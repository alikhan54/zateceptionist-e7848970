...
export function useEstimationProjects(searchTerm?: string, statusFilter?: string) {
  ...
  const { data: projects = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      console.log("[ESTIMATION] Fetching projects for tenant:", tenantId, "filter:", statusFilter);
      let query = supabase
      ...
      console.log("[ESTIMATION] Query result:", data?.length ?? 0, "projects, error:", error);
  ...
  const createProject = useMutation({
    mutationFn: async (project: Partial<EstimationProject>) => {
      console.log("[ESTIMATION] createProject called with:", JSON.stringify(project).slice(0, 500));
      console.log("[ESTIMATION] tenantId:", tenantId);
      if (!tenantId) {
        throw new Error("tenantId is null — are you logged in as the right tenant?");
      }
      const response = await estimationAction("create_project", project as Record<string, unknown>, tenantId);
      console.log("[ESTIMATION] webhook response:", JSON.stringify(response).slice(0, 500));
      if (!response.success) {
    ...
    onSuccess: (data) => {
      console.log("[ESTIMATION] createProject SUCCESS:", data);
      queryClient.invalidateQueries({ queryKey: ["estimation_projects", tenantId] });
    },
    onError: (error) => {
      console.error("[ESTIMATION] createProject ERROR:", error);
    },
  });
