...
- **PROD POST + LIVE-RUN — deferred/auto-retry:** during the session n8n's API was 503 for ~1h ("Database connection timed out" — Supabase pooler 6543 degraded; `read_only=off` so this was a connection-timeout/saturation T20, not the T18 read-only flip). Did NOT restart n8n / switch ports / touch compose. A background watcher runs `p13_post_test.py` (POST both INACTIVE → activate → seed → invoke each webhook → assert snapshot/reminders → assert isolation + no-existing-workflow-modified → deactivate → cleanup) the moment n8n returns healthy. **Workflow IDs + live-run results to be appended on recovery.**
