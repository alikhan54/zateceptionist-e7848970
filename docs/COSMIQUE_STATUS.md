...
- [ ] **Trigger Lovable Publish (Phase 7)** — `7328f28` (photos tab + AI Rx suggest) pushed but bundle hash unchanged after 6× 30s polls. Once published, run `npx playwright test --project=phase7` to flip C.1 + D.1 from DEPLOY_PENDING to REAL_PASS.
- [ ] **(Optional, Phase 8) MEDICA tool-routing tweak** — Q2 captured reply said "1 active customer and 3 leads" instead of "3 patients" (3 patients seeded). MEDICA is pulling from `customers`/`sales_leads`; should prefer `clinic_patients` for healthcare_clinic tenants. Not a regression — Phase 5c MEDICA fix was a different bug.
