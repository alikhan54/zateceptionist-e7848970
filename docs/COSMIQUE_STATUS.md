...
- [ ] **First real upload — verify n8n Doctor Avatar v1.0 workflow** — workflow is active with 0 historical executions. After Publish, do one real PDF upload (no webhook stub) and watch `docker logs 420-musetalk-service` to confirm the full MEDICA → MuseTalk → mp4 pipeline runs end-to-end. Will be the first time this pipeline runs from the frontend in 30+ days.
