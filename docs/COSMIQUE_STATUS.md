...
- [x] ~~First real upload — verify n8n Doctor Avatar v1.0 workflow~~ — done Phase 6.5. **Verdict: 🔴 WORKFLOW_BROKEN.** DA.2 validation node rejects all real uploads (reads from wrong JSON path). 1-line workflow fix queued below.
- [ ] **APPROVE 1-line n8n fix for workflow `lhdU0HUxmdgSSDpD` node DA.2** — current behavior: webhook receives `{tenant_id, patient_id, report_type, pdf_base64, pdf_filename}` and validation returns `required: {tenant_id:false, patient_id:false, report_type:false}`. Fix: change field-extraction expression in DA.2 to read from `$json.body.X` (or whatever the correct n8n webhook payload path is). Sacred workflow — needs explicit user approval before edit.
